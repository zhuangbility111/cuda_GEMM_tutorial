...
__global__ void gemm_v4_2d_block_tiling_2d_thread_tiling(float *A, float *B, float *C, int M, int N, int K) {
    ...
    for (int shared_mem_tile_idx = 0; shared_mem_tile_idx < num_shared_mem_tiles; shared_mem_tile_idx++) {
        ...
        for (int k_idx = 0; k_idx < BLOCK_TILE_SIZE_K; k_idx++) {
            ...
            for (int reg_tile_row_idx = 0; reg_tile_row_idx < THREAD_TILE_SIZE_Y; reg_tile_row_idx++) {
                A_reg_tile[reg_tile_row_idx] = A_shared_mem_tile[A_reg_tile_row_idx + reg_tile_row_idx][A_reg_tile_col_idx];
            }
    ...
    for (int reg_tile_row_idx = 0; reg_tile_row_idx < THREAD_TILE_SIZE_Y; reg_tile_row_idx++) {
        for (int reg_tile_col_idx = 0; reg_tile_col_idx < THREAD_TILE_SIZE_X; reg_tile_col_idx++) {
            ...
            int C_col_idx = blockIdx.x * BLOCK_TILE_SIZE_X + thread_linear_idx % (BLOCK_TILE_SIZE_X / THREAD_TILE_SIZE_X) * THREAD_TILE_SIZE_X + reg_tile_col_idx;
            if (C_row_idx < M && C_col_idx < N) {
                C[C_row_idx * N + C_col_idx] = C_reg_tile[reg_tile_row_idx][reg_tile_col_idx];
            }
        }
    }
}
...
void launch_gemm_v4_2d_block_tiling_2d_thread_tiling(float *A, float *B, float *C, int M, int N, int K) {
    ...
    gemm_v4_2d_block_tiling_2d_thread_tiling<BLOCK_TILE_SIZE_X, BLOCK_TILE_SIZE_Y, BLOCK_TILE_SIZE_K,
